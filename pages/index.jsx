...
function Quiz({ onAnswer }) {
  ...
  const collocationPool = useMemo(() => sentencePool.filter(x => (x.collocations && x.collocations.length) || chs(x.word).length === 4), [sentencePool]);
  ...
  const newExamQ = useCallback(() => {
    ...
    setSq({ kind: 'exam', badge: '填空 · Fill in the blank', prompt: 'Choose the correct idiom.', sentence: cor.cn.replace(cor.word, '＿＿＿＿'),
  ...
  const newCollocationQ = useCallback(() => {
    ...
    setSq({ kind: 'collocation', badge: '词语搭配 · Collocation', prompt: '选出与句子搭配得当的词语。', sub: hint,
