  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9 }}>
        {[
          { tab:'review', icon:'📖', label:'Start Review', sub: due > 0 ? `${due} cards due` : 'New cards waiting', hi:true },
          { tab:'quiz', icon:'🎯', label:'Quick Quiz', sub:'3 quiz modes' },
          ...
          { tab:'chars', icon:'🔤', label:'Character Groups', sub:'词语替换 practice' },
...
function Quiz({ onAnswer }) {
  ...
  const newEQ = useCallback(() => {
    const pool = shuffle(IDIOMS); const cor = pool[0];
    setExamQ({ sentence: cor.cn.replace(cor.word, '______'), word: cor.word, meaning: cor.meaning, opts: shuffle([cor, ...pool.slice(1, 4)]).map(o => ({ word: o.word, isC: o.word === cor.word })) });
    setExamSel(null);
  }, []);

  const loadAi = async () => {
    setAiL(true); setAiQ(null); setAiSel(null);
    const pool = shuffle(IDIOMS).slice(0, 8); const cor = pool[0];
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 600,
          system: 'Chinese vocab quiz. Return ONLY valid JSON no markdown.',
          messages: [{ role: 'user', content: `Fill-in-blank for "${cor.word}"(${cor.meaning}). New Chinese sentence (not: "${cor.cn}"), replace idiom with ___. Distractors: ${pool.slice(1,4).map(i=>i.word).join(', ')}. JSON: {"question":"...___...","english":"translation","options":["A. ${cor.word}","B. ...","C. ...","D. ..."],"answer":"A","explanation":"..."}` }] })
      });
      const d = await res.json();
      setAiQ(JSON.parse(d.content[0].text.replace(/\`\`\`json|\`\`\`/g, '').trim()));
    } catch { setAiQ({ error: true }); }
    setAiL(false);
  };

  const qBtn = { display:'block', width:'100%', textAlign:'left', padding:'10px 13px', margin:'5px 0', border:'1px solid #E8E8E4', borderRadius:9, background:'transparent', cursor:'pointer', fontSize:13, color:'#1A1A18', boxSizing:'border-box' };
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
        {[
          ...
          { id:'exam', icon:'📝', title:'Exam Fill-in-Blank', sub:'Real sentences with gaps', action: () => { setMode('exam'); newEQ(); } },
          { id:'ai', icon:'✨', title:'AI Sentence Quiz', sub:'AI-generated harder sentences', action: () => { setMode('ai'); loadAi(); } },
  ...
  if (mode === 'exam' && examQ) return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12, alignItems:'center' }}><BackBtn /><ScoreTag /></div>
      <Card>
        <Badge type="关联词" style={{ marginBottom:9, display:'inline-block' }}>Fill in the blank</Badge>
        <p style={{ fontSize:15, fontWeight:600, color:'#1A1A18', margin:'6px 0 4px', lineHeight:1.7 }}>{examQ.sentence}</p>
        <p style={{ fontSize:11, color:'#888', marginBottom:12 }}>Choose the correct idiom.</p>
        {examQ.opts.map((opt, i) => {
          const sty = examSel !== null ? (opt.isC ? { background:'#EAF3DE', borderColor:'#97C459', color:'#3B6D11' } : examSel === i ? { background:'#FCEBEB', borderColor:'#E24B4A', color:'#A32D2D' } : { opacity:0.5 }) : {};
          return <button key={i} style={{ ...qBtn, ...sty }} onClick={() => { if (examSel !== null) return; setExamSel(i); onAnswer(); setScore(s => ({ c: s.c + (opt.isC ? 1 : 0), t: s.t + 1 })); }}><span style={{ fontFamily:'serif', fontSize:15 }}>{opt.word}</span></button>;
        })}
        {examSel !== null && (
          <>
            <div style={{ background:'#FAFAF8', borderRadius:8, padding:'8px 11px', marginTop:10, border:'1px solid #F0F0EC' }}>
              <p style={{ fontSize:12, fontWeight:600, color: examQ.opts[examSel]?.isC ? '#3B6D11' : '#A32D2D' }}>{examQ.opts[examSel]?.isC ? `✓ Correct! "${examQ.word}" — ${examQ.meaning}` : `✗ Incorrect. Answer: "${examQ.word}" — ${examQ.meaning}`}</p>
            </div>
            <NextBtn onClick={newEQ} />
          </>
        )}
      </Card>
    </div>
  ...
  if (mode === 'ai') return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12, alignItems:'center' }}><BackBtn /><ScoreTag /></div>
      {aiL && <div style={{ textAlign:'center', padding:'48px 0', color:'#AAA', fontSize:13 }}>Generating with AI…</div>}
      {!aiL && aiQ?.error && <Card><p style={{ color:'#AAA', fontSize:13, marginBottom:10 }}>Could not load question.</p><button onClick={loadAi} style={{ fontSize:12, padding:'7px 14px', border:'1px solid #E0E0DC', borderRadius:7, background:'transparent', cursor:'pointer' }}>Retry</button></Card>}
      {!aiL && aiQ && !aiQ.error && (
        <Card>
          <Badge type="关联词" style={{ marginBottom:9, display:'inline-block' }}>AI fill in the blank</Badge>
          <p style={{ fontSize:15, fontWeight:600, color:'#1A1A18', margin:'6px 0 3px', lineHeight:1.7 }}>{aiQ.question}</p>
          {aiQ.english && <p style={{ fontSize:11, color:'#AAA', marginBottom:12, fontStyle:'italic' }}>{aiQ.english}</p>}
          {aiQ.options.map((opt, i) => {
            const lt = opt[0], iC = lt === aiQ.answer, iS = aiSel === lt;
            const sty = aiSel ? (iC ? { background:'#EAF3DE', borderColor:'#97C459', color:'#3B6D11' } : iS ? { background:'#FCEBEB', borderColor:'#E24B4A', color:'#A32D2D' } : { opacity:0.5 }) : {};
            return <button key={i} style={{ ...qBtn, ...sty }} onClick={() => { if (aiSel) return; setAiSel(lt); onAnswer(); setScore(s => ({ c: s.c + (iC ? 1 : 0), t: s.t + 1 })); }}>{opt}</button>;
          })}
          {aiSel && (
            <>
              <div style={{ background:'#FAFAF8', borderRadius:8, padding:'8px 11px', marginTop:10, border:'1px solid #F0F0EC' }}>
                <p style={{ fontSize:12, fontWeight:600, color: aiSel === aiQ.answer ? '#3B6D11' : '#A32D2D' }}>{aiSel === aiQ.answer ? '✓ Correct!' : `✗ Incorrect — answer: ${aiQ.answer}`}</p>
                <p style={{ fontSize:11, color:'#888', marginTop:2 }}>{aiQ.explanation}</p>
              </div>
              <NextBtn onClick={loadAi} />
            </>
          )}
        </Card>
      )}
  ...
  return (
    <div>
      <div style={{ background:'#FFFBF5', border:'1px solid #F5DEC8', borderRadius:9, padding:'9px 13px', marginBottom:12, fontSize:12, color:'#8A5E00' }}>
        {EXAM_WORDS.length} words from O-level Chinese papers, 2014–2025.
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(120px,1fr))', gap:7 }}>
        {paged.map((w, i) => {
          ...
          return (
            <div key={i} style={{ background:'white', border:'1px solid #E8E8E4', borderRadius:10, padding:'10px 11px' }}>
  ...
  return (
    <div>
      <div style={{ background:'#F0F7FF', border:'1px solid #C5DEF5', borderRadius:9, padding:'9px 13px', marginBottom:12, fontSize:12, color:'#185FA5' }}>
        {SYLLABUS_WORDS.length} words from the Sec 1–4 syllabus. Orange dot = also appeared in past exams.
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(125px,1fr))', gap:7 }}>
        {paged.map((w, i) => {
          ...
          return (
            <div key={i} style={{ background:'white', border:`1px solid ${inExam ? '#F5DEC8' : '#E8E8E4'}`, borderRadius:10, padding:'10px 11px', position:'relative' }}>
  ...
  return (
    <div>
      <div style={{ background:'#FAFAF8', border:'1px solid #E8E8E4', borderRadius:9, padding:'9px 13px', marginBottom:12, fontSize:12, color:'#888', lineHeight:1.6 }}>
        Words sharing a character often appear in <strong>词语替换</strong> questions. Tap any group to expand.
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(145px,1fr))', gap:8 }}>
        {groups.map((g, i) => (
          <Card key={i} style={{ padding:'11px 13px', cursor:'pointer', border: open === i ? `2px solid ${A}` : '1px solid #E8E8E4' }} onClick={() => setOpen(open === i ? null : i)}>
            ...
            {open === i
              ? <div style={{ display:'flex', flexWrap:'wrap', gap:3 }}>{g.words.map((w, j) => <span key={j} style={{ fontSize:11, fontFamily:'serif', background:'#F5F5F2', padding:'2px 6px', borderRadius:20, color:'#1A1A18' }}>{w}</span>)}</div>
              : <div style={{ fontSize:11, color:'#888' }}>{g.words.slice(0, 3).join('、')}…</div>}
          </Card>
        ))}
      </div>
    </div>
    ...
    <div>
      ...
      <Card>
        ...
        <p style={{ fontSize:12, color:'#888', lineHeight:1.7 }}>
          <strong>103</strong> idioms with SM-2 spaced repetition · <strong>{EXAM_WORDS.length}</strong> past exam words (2014–2025) · <strong>{SYLLABUS_WORDS.length}</strong> syllabus words (Sec 1–4) · AI quiz · Character groups for 词语替换 practice.
...
const TITLES = { home:'华文词汇练习', review:'Flashcard Review', quiz:'Quiz', examwords:'Past Exam Words', syllabus:'Syllabus Words', chars:'Character Groups', more:'Progress & Export' };
