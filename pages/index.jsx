    ...
    <div>
      ...
      <Card>
        ...
        {q.opts.map((opt, i) => {
          ...
          return <button key={(q.qid||0)+":"+i} style={{ ...qBtn, ...sty }} onMouseDown={e => e.preventDefault()} onClick={e => { if (sel !== null) return; e.currentTarget.blur(); setSel(i); onAnswer(); setScore(s => ({ c: s.c + (opt.isC ? 1 : 0), t: s.t + 1 })); }}>{opt.label}{opt.sub && <span style={{ fontSize:11, opacity:0.6, marginLeft:5 }}>{opt.sub}</span>}</button>;
        })}
      ...
      <div>
        ...
        <Card>
          ...
          {sq.opts.map((opt, i) => {
            ...
            return <button key={(sq.qid||0)+":"+i} style={{ ...qBtn, ...sty }} onMouseDown={e => e.preventDefault()} onClick={e => { if (sqSel !== null) return; e.currentTarget.blur(); setSqSel(i); onAnswer(); setScore(s => ({ c: s.c + (opt.isC ? 1 : 0), t: s.t + 1 })); }}><span style={{ fontFamily:'serif', fontSize:16 }}>{opt.word}</span></button>;
          })}
          {sqSel !== null && (
            <>
